...
for BATCH_SIZE in 32 64 128 256; do
    MAX_EPOCHS=300
    NUM_WORKERS=16
    EXP_NAME="default_bs${BATCH_SIZE}ep300"
    ...
    python3 -u "$NAVSIM_DEVKIT_ROOT"/planning/script/run_training.py \
        --config-name=default_training_w_callbacks \
        ...
        force_cache_computation=False 2>&1 | tee -a "$LOG_FILE"
